...
if "messaggi" in tab_map:
    with tab_map["messaggi"]:
        if df is None:
            ...
            st.markdown("## Centro messaggi")
            top_action_col1, top_action_col2 = st.columns([3, 1])

            with top_action_col1:
                st.caption("Genera, monitora e invia i messaggi automatici delle prenotazioni.")
            with top_action_col2:
                st.caption("Aggiorna dopo ogni modifica ai template.")
            ...
            st.info("Se modifichi template o orari, premi 'Genera / aggiorna messaggi programmati' per aggiornare i messaggi già creati.")
            ...
            if len(valid) == 0:
                ...
                if scheduled_df.empty:
                    st.info("Nessun messaggio programmato ancora. Premi il pulsante sopra per generarli.")
                else:
                    ...
                    with metric1:
                        st.markdown(f"""
                        <div style="padding:18px;border:1px solid #5b4b00;border-radius:18px;background:rgba(255,193,7,0.08);">
                            <div style="font-size:13px;color:#facc15;">🟡 Da inviare</div>
                            <div style="font-size:38px;font-weight:700;">{pending_count}</div>
                        </div>
                        """, unsafe_allow_html=True)

                    with metric2:
                        st.markdown(f"""
                        <div style="padding:18px;border:1px solid #14532d;border-radius:18px;background:rgba(34,197,94,0.08);">
                            <div style="font-size:13px;color:#4ade80;">🟢 Inviati</div>
                            <div style="font-size:38px;font-weight:700;">{sent_count}</div>
                        </div>
                        """, unsafe_allow_html=True)

                    with metric3:
                        st.markdown(f"""
                        <div style="padding:18px;border:1px solid #7f1d1d;border-radius:18px;background:rgba(239,68,68,0.08);">
                            <div style="font-size:13px;color:#f87171;">🔴 Falliti</div>
                            <div style="font-size:38px;font-weight:700;">{failed_count}</div>
                        </div>
                        """, unsafe_allow_html=True)

                    with metric4:
                        st.markdown(f"""
                        <div style="padding:18px;border:1px solid #374151;border-radius:18px;background:rgba(107,114,128,0.08);">
                            <div style="font-size:13px;color:#d1d5db;">⚪ Annullati</div>
                            <div style="font-size:38px;font-weight:700;">{cancelled_count}</div>
                        </div>
                        """, unsafe_allow_html=True)

                    st.markdown("### Filtri messaggi")
                    ...
                    if not display_df.empty:
                        ...
                        with left_col:
                            ...
                            for _, row in display_df.iterrows():
                                ...
                                labels_by_id[int(row["id"])] = (
                                    f"{badge} #{int(row['id'])} · {row['guest_name']} · "
                                    f"{label_message_type(row['message_type'])} · "
                                    f"{pd.to_datetime(row['send_at']).strftime('%d/%m/%Y %H:%M')}"
                                )
                        ...
                        with right_col:
                            ...
                            if current_msg:
                                st.markdown("### Dettaglio messaggio")

                                current_status = str(current_msg["status"]).lower()

                                if current_status == "pending":
                                    status_label = "🟡 In attesa di invio"
                                    status_color = "#facc15"
                                elif current_status == "sent":
                                    sent_time = pd.to_datetime(current_msg["sent_at"], errors="coerce")
                                    sent_label = sent_time.strftime("%d/%m/%Y %H:%M") if pd.notnull(sent_time) else "inviato"
                                    status_label = f"🟢 Inviato il {sent_label}"
                                    status_color = "#4ade80"
                                elif current_status == "failed":
                                    status_label = "🔴 Errore invio WhatsApp"
                                    status_color = "#f87171"
                                else:
                                    status_label = "⚪ Messaggio annullato"
                                    status_color = "#d1d5db"

                                st.markdown(f"""
                                <div style="padding:18px;border-radius:18px;border:1px solid {status_color};margin-bottom:12px;">
                                    <div style="font-size:14px;color:{status_color};font-weight:600;">{status_label}</div>
                                    <div style="font-size:28px;font-weight:700;margin-top:6px;">{current_msg["guest_name"]}</div>
                                    <div style="font-size:14px;opacity:0.8;margin-top:6px;">
                                        {label_message_type(current_msg["message_type"])} · {current_msg["channel"]} · Invio previsto: {pd.to_datetime(current_msg["send_at"]).strftime("%d/%m/%Y %H:%M")}
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                with info1:
                                    st.text_input("Telefono", value=str(current_msg.get("guest_phone", "") or ""), disabled=True)
                                with info2:
                                    st.text_input("Piattaforma", value=str(current_msg.get("platform", "") or ""), disabled=True)
                                ...
                                if current_msg.get("error_message"):
                                    st.warning(str(current_msg.get("error_message")))
                                ...
                                if current_status == "failed":
                                    ...
                                    with action2:
                                        if st.button("Ripristina pending", use_container_width=True, key=f"retry_msg_{selected_id}"):
                                            ...
                                            if ok:
                                                st.success("Messaggio rimesso in coda.")
                                                st.rerun()

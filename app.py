...
def replace_scheduled_messages_for_user(utente_id, bookings_df, profilo, scheduling_rules=None, template_base=None):
            ...
            if existing:
                if existing["status"] in ["sent", "cancelled"]:
                    cur.execute(
                        ...
                        SET platform = ?, guest_name = ?, guest_phone = ?, check_in = ?, check_out = ?, send_at = ?, channel = ?
                        WHERE id = ?
                        """,
                        (
                            msg["platform"],
                    ...
                    cur.execute(
                        ...
                        SET platform = ?, guest_name = ?, guest_phone = ?, check_in = ?, check_out = ?,
                            message_text = ?, send_at = ?, channel = ?, status = 'pending', error_message = NULL
                        WHERE id = ?
                        """,
                        (
                            msg["platform"],
    ...
    for row in existing_rows:
        key = (row["booking_ref"], row["message_type"])

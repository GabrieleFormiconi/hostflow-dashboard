...
def resolve_message_guest_phone(current_msg, bookings_df):
    phone = str((current_msg or {}).get("guest_phone", "") or "").strip()
    if phone:
        return phone

    if bookings_df is None or len(bookings_df) == 0 or not current_msg:
        return ""

    booking_ref_value = str(current_msg.get("booking_ref", "") or "").strip()
    if booking_ref_value:
        try:
            matches = bookings_df[bookings_df.apply(booking_reference, axis=1) == booking_ref_value]
            if not matches.empty:
                matched_phone = str(matches.iloc[0].get("guest_phone", "") or "").strip()
                if matched_phone:
    ...
    try:
        ...
        if not fallback.empty:
            matched_phone = str(fallback.iloc[0].get("guest_phone", "") or "").strip()
            if matched_phone:
                return matched_phone
    except Exception:
        pass

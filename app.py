...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS whatsapp_accounts (
            ...
            token_last4 TEXT,
            embedded_signup_status TEXT DEFAULT 'not_connected',
    ...
    for col_name, col_type in whatsapp_columns:
        try:
            cur.execute(f"ALTER TABLE whatsapp_accounts ADD COLUMN {col_name} {col_type}")
        except sqlite3.OperationalError:
            pass
...
APP_ENCRYPTION_KEY = str(get_secret_value("APP_ENCRYPTION_KEY", "hostflow_local_test_key") or "hostflow_local_test_key")
WHATSAPP_EMBEDDED_SIGNUP_URL = str(get_secret_value("WHATSAPP_EMBEDDED_SIGNUP_URL", "") or "").strip()
HOSTFLOW_SUPPORT_EMAIL = str(get_secret_value("HOSTFLOW_SUPPORT_EMAIL", "") or "").strip()
ENABLE_WHATSAPP_TECH_SETUP = str(get_secret_value("ENABLE_WHATSAPP_TECH_SETUP", "true") or "true").strip().lower() in ["1", "true", "yes", "si", "sì", "y"]
...
def encrypt_sensitive_value(value):
    ...
    if not raw:
        return ""
    key_bytes = hashlib.sha256(APP_ENCRYPTION_KEY.encode("utf-8")).digest()
    encrypted = _xor_bytes(raw.encode("utf-8"), key_bytes)
...
def decrypt_sensitive_value(value):
    ...
    if not raw:
        return ""
    try:
        encrypted = base64.urlsafe_b64decode(raw.encode("utf-8"))
        key_bytes = hashlib.sha256(APP_ENCRYPTION_KEY.encode("utf-8")).digest()
        decrypted = _xor_bytes(encrypted, key_bytes)
...
def default_whatsapp_config():
    token = str(WHATSAPP_ACCESS_TOKEN or "").strip()
    phone_number_id = str(WHATSAPP_PHONE_NUMBER_ID or "").strip()
    business_account_id = str(WHATSAPP_BUSINESS_ACCOUNT_ID or "").strip()
    if not token or not phone_number_id:
        return None
    return {
        "connection_mode": "platform_default",
        "business_display_name": "HostFlow default",
        "business_phone_number": "",
        "phone_number_id": phone_number_id,
        "business_account_id": business_account_id,
        "access_token": token,
        "token_last4": token[-4:] if len(token) >= 4 else token,
        "embedded_signup_status": "fallback_secret",
...
def save_whatsapp_config(utente_id, config):
    access_token = str(config.get("access_token", "") or "").strip()
    phone_number_id = str(config.get("phone_number_id", "") or "").strip()
    business_account_id = str(config.get("business_account_id", "") or "").strip()
    business_display_name = str(config.get("business_display_name", "") or "").strip()
    business_phone_number = normalize_whatsapp_phone(config.get("business_phone_number", ""))
    connection_mode = str(config.get("connection_mode", "manual") or "manual").strip() or "manual"

    status = "connected" if access_token and phone_number_id else "not_connected"
    encrypted_token = encrypt_sensitive_value(access_token) if access_token else ""
    token_last4 = access_token[-4:] if len(access_token) >= 4 else access_token
    connected_at = datetime.utcnow().isoformat() if access_token and phone_number_id else None
    ...
    cur.execute(
        ...
        INSERT INTO whatsapp_accounts (
            utente_id, connection_mode, business_display_name, business_phone_number,
            phone_number_id, business_account_id, access_token_encrypted, token_last4,
            embedded_signup_status, webhook_status, last_test_status, last_test_message,
        ...
        ON CONFLICT(utente_id) DO UPDATE SET
            ...
            embedded_signup_status = excluded.embedded_signup_status,
...
def update_whatsapp_test_status(utente_id, success, message):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        """
        ...
        SET last_test_status = ?,
            last_test_message = ?,
            webhook_status = CASE WHEN ? = 1 THEN 'reachable' ELSE webhook_status END,
            updated_at = CURRENT_TIMESTAMP
        WHERE utente_id = ?
        """,
        (
...
def disconnect_whatsapp_config(utente_id):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        """
...
def get_whatsapp_config(utente_id, allow_default=True):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        """
        SELECT connection_mode, business_display_name, business_phone_number,
               phone_number_id, business_account_id, access_token_encrypted,
               token_last4, embedded_signup_status, webhook_status,
               last_test_status, last_test_message, connected_at, updated_at
    ...
    if row:
        data = dict(row)
        data["access_token"] = decrypt_sensitive_value(data.get("access_token_encrypted", ""))
        data["is_configured"] = bool(data.get("access_token") and data.get("phone_number_id"))
        data["uses_platform_default"] = False
        return data

    fallback = default_whatsapp_config() if allow_default else None
    if fallback:
        fallback["is_configured"] = True
        fallback["uses_platform_default"] = True
        return fallback
    ...
    return {
        ...
        "access_token": "",
        "token_last4": "",
        "embedded_signup_status": "not_connected",
        "webhook_status": "pending",
        "last_test_status": None,
        "last_test_message": None,
        "connected_at": None,
...
def get_whatsapp_embedded_signup_url(utente_id=None):
    base_url = str(WHATSAPP_EMBEDDED_SIGNUP_URL or "").strip()
    if not base_url:
        return ""
    ...
    return f"{base_url}{separator}hostflow_user_id={int(utente_id)}"


def whatsapp_embedded_signup_available():
    return bool(str(WHATSAPP_EMBEDDED_SIGNUP_URL or "").strip())


def whatsapp_support_caption():
    if HOSTFLOW_SUPPORT_EMAIL:
        return f"Se il bottone automatico non si apre o il collegamento non va a buon fine, contatta il supporto: {HOSTFLOW_SUPPORT_EMAIL}"
    return "Se il bottone automatico non si apre o il collegamento non va a buon fine, contatta il supporto della piattaforma."


def send_whatsapp_message(utente_id, phone_number, message_text):
    config = get_whatsapp_config(utente_id)
    access_token = str(config.get("access_token", "") or "").strip()
    phone_number_id = str(config.get("phone_number_id", "") or "").strip()

    if not access_token or not phone_number_id:
        return False, "Configurazione WhatsApp mancante per questo account"

    clean_phone = normalize_whatsapp_phone(phone_number)
    if not clean_phone:
    ...
    try:
        ...
        update_whatsapp_test_status(utente_id, success, data)
        if success:
            return True, data
        return False, f"HTTP {response.status_code} - {data}"
    except Exception as e:
        update_whatsapp_test_status(utente_id, False, str(e))
...
def render_profile_form(profilo, onboarding_mode=False):
    ...
    whatsapp_config = get_whatsapp_config(st.session_state.utente["id"])
    ...
    st.markdown("#### WhatsApp Business per invio messaggi")
    current_whatsapp_config = get_whatsapp_config(st.session_state.utente["id"])
    status_label_map = {
        "connected": "Connesso",
        "not_connected": "Non collegato",
        "fallback_secret": "Default piattaforma",
    }
    status_value = status_label_map.get(
        current_whatsapp_config.get("embedded_signup_status"),
        current_whatsapp_config.get("embedded_signup_status", "Non collegato"),
    )

    if current_whatsapp_config.get("is_configured"):
        st.success(f"WhatsApp collegato: {status_value}")
        if current_whatsapp_config.get("uses_platform_default"):
            st.caption("Questo account sta ancora usando la configurazione di default della piattaforma. Per un cliente reale collega un account WhatsApp Business dedicato.")
        else:
            summary_items = []
            if current_whatsapp_config.get("business_display_name"):
                summary_items.append(f"Business: {current_whatsapp_config.get('business_display_name')}")
            if current_whatsapp_config.get("business_phone_number"):
                summary_items.append(f"Numero collegato: +{current_whatsapp_config.get('business_phone_number')}")
            if summary_items:
                st.caption(" · ".join(summary_items))
            if current_whatsapp_config.get("last_test_status") == "success":
                st.caption("Ultimo test collegamento: riuscito")
            elif current_whatsapp_config.get("last_test_status") == "failed":
                st.caption("Ultimo test collegamento: fallito")
    else:
        st.warning("WhatsApp non ancora collegato per questo account.")

    with st.container(border=True):
        st.markdown("### Collegamento automatico")
        st.write("Il cliente finale non deve conoscere token, ID o impostazioni tecniche. Deve solo premere il bottone qui sotto, accedere con Meta e completare il collegamento del proprio numero WhatsApp Business.")

        embedded_signup_url = get_whatsapp_embedded_signup_url(st.session_state.utente["id"])
        if whatsapp_embedded_signup_available():
            st.link_button(
            ...
            st.caption("Il bottone apre il flusso automatico Meta. Una volta completato, torna su HostFlow e usa il test collegamento qui sotto.")
        else:
            st.info("Il flusso automatico Meta non è ancora configurato in questo ambiente. Per i test interni puoi usare la modalità tecnica nascosta qui sotto, ma per i clienti finali va configurato una sola volta lato piattaforma.")

        st.caption(whatsapp_support_caption())

        test_col1, test_col2 = st.columns([1, 1])
        with test_col1:
            test_whatsapp_saved_clicked = st.button(
                "Testa collegamento salvato",
                use_container_width=True,
                key=f"test_saved_whatsapp_config_{'onboarding' if onboarding_mode else 'tab'}",
            )
        with test_col2:
            disconnect_whatsapp_clicked = st.button(
                "Disconnetti WhatsApp",
                use_container_width=True,
                key=f"disconnect_whatsapp_config_{'onboarding' if onboarding_mode else 'tab'}",
            )

        if test_whatsapp_saved_clicked:
            sender_number = current_whatsapp_config.get("business_phone_number", "")
            if not current_whatsapp_config.get("is_configured") or current_whatsapp_config.get("uses_platform_default"):
                st.error("Questo account non ha ancora un collegamento WhatsApp dedicato salvato. Completa prima il collegamento automatico.")
            elif not sender_number:
                st.error("Manca il numero WhatsApp Business collegato. Completa di nuovo il collegamento o usa la modalità tecnica.")
            else:
                success, result = send_whatsapp_message(
                    st.session_state.utente["id"],
                    sender_number,
                    "Test collegamento HostFlow completato. Questo messaggio conferma che il tuo account WhatsApp Business è pronto per l'invio multi-cliente.",
                )
                st.session_state.whatsapp_config = get_whatsapp_config(st.session_state.utente["id"])
                if success:
                    st.success("Test WhatsApp riuscito. Il collegamento è attivo.")
                else:
                    st.error(f"Test WhatsApp fallito: {result}")

        if disconnect_whatsapp_clicked:
            if delete_whatsapp_config(st.session_state.utente["id"]):
                st.session_state.whatsapp_config = get_whatsapp_config(st.session_state.utente["id"])
                st.success("Collegamento WhatsApp rimosso da questo account.")
                st.rerun()

    if ENABLE_WHATSAPP_TECH_SETUP:
        with st.expander("Modalità tecnica / fallback admin", expanded=False):
            st.caption("Questa sezione è pensata per test interni o supporto amministrativo. Il cliente finale non dovrebbe dover inserire questi dati manualmente.")
            wa_business_display_name = st.text_input(
                "Nome business WhatsApp",
                value=str(current_whatsapp_config.get("business_display_name", "") or ""),
                key=f"wa_business_display_name_{'onboarding' if onboarding_mode else 'tab'}",
            )
            wa_business_phone_number = st.text_input(
                "Numero WhatsApp Business mittente",
                value=(f"+{current_whatsapp_config.get('business_phone_number')}" if current_whatsapp_config.get("business_phone_number") else ""),
                key=f"wa_business_phone_number_{'onboarding' if onboarding_mode else 'tab'}",
                help="Usa prefisso internazionale, es. +393XXXXXXXXX",
            )
            wa_phone_number_id = st.text_input(
                "Phone Number ID",
                value=str(current_whatsapp_config.get("phone_number_id", "") or ""),
                key=f"wa_phone_number_id_{'onboarding' if onboarding_mode else 'tab'}",
            )
            wa_business_account_id = st.text_input(
                "Business Account ID (opzionale)",
                value=str(current_whatsapp_config.get("business_account_id", "") or ""),
                key=f"wa_business_account_id_{'onboarding' if onboarding_mode else 'tab'}",
            )
            wa_access_token = st.text_input(
                "Access Token Meta",
                value="",
                type="password",
                placeholder="Incolla qui il token del cliente per salvarlo/aggiornarlo",
                key=f"wa_access_token_{'onboarding' if onboarding_mode else 'tab'}",
                help="Per sicurezza il token salvato non viene mostrato. Se lasci vuoto, resta quello già salvato.",
            )

            wa_col1, wa_col2 = st.columns(2)
            with wa_col1:
                save_whatsapp_clicked = st.button(
                    "Salva collegamento manuale",
                    use_container_width=True,
                    key=f"save_whatsapp_config_{'onboarding' if onboarding_mode else 'tab'}",
                )
            with wa_col2:
                test_whatsapp_clicked = st.button(
                    "Testa collegamento manuale",
                    use_container_width=True,
                    key=f"test_whatsapp_config_{'onboarding' if onboarding_mode else 'tab'}",
                )

            if save_whatsapp_clicked:
                existing_token = current_whatsapp_config.get("access_token", "") if not current_whatsapp_config.get("uses_platform_default") else ""
                token_to_save = str(wa_access_token or "").strip() or existing_token
                if not str(wa_phone_number_id or "").strip():
                    st.error("Inserisci il Phone Number ID del cliente.")
                elif not token_to_save:
                    st.error("Inserisci l'Access Token Meta del cliente.")
                else:
                    save_whatsapp_config(
                        st.session_state.utente["id"],
                        {
                            "connection_mode": "manual",
                            "business_display_name": wa_business_display_name,
                            "business_phone_number": wa_business_phone_number,
                    ...
                    st.session_state.whatsapp_config = get_whatsapp_config(st.session_state.utente["id"])
                    st.success("Collegamento WhatsApp manuale salvato per questo account.")
                    st.rerun()

            if test_whatsapp_clicked:
                existing_token = current_whatsapp_config.get("access_token", "") if not current_whatsapp_config.get("uses_platform_default") else ""
                token_to_test = str(wa_access_token or "").strip() or existing_token
                normalized_sender_number = normalize_whatsapp_phone(wa_business_phone_number)
                if not str(wa_phone_number_id or "").strip() or not token_to_test:
                    st.error("Per testare inserisci almeno Phone Number ID e Access Token.")
                elif not normalized_sender_number:
                    st.error("Inserisci il numero WhatsApp Business mittente per eseguire il test su te stessa.")
                else:
                    save_whatsapp_config(
                        st.session_state.utente["id"],
                        {
                                ...
                                if status_value == "failed":
                                    ...
                                    with action1:
                                        if st.button("Invia WhatsApp ora", use_container_width=True, key=f"send_now_{selected_id}"):
                                            ...
                                            success, result = send_whatsapp_message(st.session_state.utente["id"], phone_number, message_text)
                                            if success:

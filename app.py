...
def delete_custom_booking(utente_id, booking_id):
    ...
    if changed > 0 and booking_row:
        booking_ref = "|".join([
            str(booking_row["platform"] or "Custom").strip() or "Custom",
            str(booking_row["guest_name"] or "").strip(),
            pd.to_datetime(booking_row["check_in"], errors="coerce").date().isoformat(),
            pd.to_datetime(booking_row["check_out"], errors="coerce").date().isoformat(),
        ])
        cur.execute(
            "DELETE FROM scheduled_messages WHERE utente_id = ? AND booking_ref = ?",
            (int(utente_id), booking_ref),
        )
...
def render_dashboard_dataframe(df_to_show, user_id):
    ...
    with st.expander("Aggiungi prenotazione custom", expanded=False):
        ...
        if custom_bookings_df.empty:
            ...
            st.caption("Le prenotazioni custom inserite e il riepilogo generale non vengono mostrati qui sotto per mantenere la sezione più pulita.")
    ...
    with tab_map["messaggi"]:
        if df is None:
            ...
            persistent_message_keys = {
                "msg_rule_confirm_offset_days": 0,
                "msg_rule_confirm_time": "10:00",
                "msg_rule_checkin_reminder_days": 1,
                "msg_rule_checkin_reminder_time": "18:00",
                "msg_rule_checkin_instr_time": "15:00",
                "msg_rule_checkout_reminder_days": 1,
                ...
                "template_checkin_reminder": "",
                "template_checkin_instructions": "",
                "template_checkout_reminder": "",
                "template_review_request": "",
            }

            for cfg_key, cfg_default in persistent_message_keys.items():
                if cfg_key not in st.session_state:
                    st.session_state[cfg_key] = saved_message_settings.get(cfg_key, cfg_default)

            template_editor_key_map = {
                "template_booking_confirmation": "template_booking_confirmation_editor",
                "template_checkin_reminder": "template_checkin_reminder_editor",
                "template_checkin_instructions": "template_checkin_instructions_editor",
                "template_checkout_reminder": "template_checkout_reminder_editor",
                "template_review_request": "template_review_request_editor",
            }

            def sync_template_from_editor(persistent_key):
                editor_key = template_editor_key_map[persistent_key]
                st.session_state[persistent_key] = st.session_state.get(editor_key, "")

            for persistent_key, editor_key in template_editor_key_map.items():
                if editor_key not in st.session_state:
                    st.session_state[editor_key] = st.session_state.get(persistent_key, saved_message_settings.get(persistent_key, ""))

            current_message_settings = {
                "msg_rule_confirm_offset_days": int(st.session_state.get("msg_rule_confirm_offset_days", saved_message_settings.get("msg_rule_confirm_offset_days", 0))),
                "msg_rule_confirm_time": str(st.session_state.get("msg_rule_confirm_time", saved_message_settings.get("msg_rule_confirm_time", "10:00"))),
                "msg_rule_checkin_reminder_days": int(st.session_state.get("msg_rule_checkin_reminder_days", saved_message_settings.get("msg_rule_checkin_reminder_days", 1))),
                "msg_rule_checkin_reminder_time": str(st.session_state.get("msg_rule_checkin_reminder_time", saved_message_settings.get("msg_rule_checkin_reminder_time", "18:00"))),
                "msg_rule_checkin_instr_time": str(st.session_state.get("msg_rule_checkin_instr_time", saved_message_settings.get("msg_rule_checkin_instr_time", "15:00"))),
                "msg_rule_checkout_reminder_days": int(st.session_state.get("msg_rule_checkout_reminder_days", saved_message_settings.get("msg_rule_checkout_reminder_days", 1))),
                "msg_rule_checkout_reminder_time": str(st.session_state.get("msg_rule_checkout_reminder_time", saved_message_settings.get("msg_rule_checkout_reminder_time", "18:00"))),
                "msg_rule_review_days_after": int(st.session_state.get("msg_rule_review_days_after", saved_message_settings.get("msg_rule_review_days_after", 1))),
                "msg_rule_review_time": str(st.session_state.get("msg_rule_review_time", saved_message_settings.get("msg_rule_review_time", "12:00"))),
                "template_booking_confirmation": st.session_state.get("template_booking_confirmation", saved_message_settings.get("template_booking_confirmation", "")),
                "template_checkin_reminder": st.session_state.get("template_checkin_reminder", saved_message_settings.get("template_checkin_reminder", "")),
                "template_checkin_instructions": st.session_state.get("template_checkin_instructions", saved_message_settings.get("template_checkin_instructions", "")),
                "template_checkout_reminder": st.session_state.get("template_checkout_reminder", saved_message_settings.get("template_checkout_reminder", "")),
                "template_review_request": st.session_state.get("template_review_request", saved_message_settings.get("template_review_request", "")),
            }
                ...
                with st.container(border=True):
                    ...
                    with template_tabs[0]:
                        st.text_area(
                            ...
                            on_change=sync_template_from_editor,
                            args=("template_booking_confirmation",),
                        )
                    with template_tabs[1]:
                        st.text_area(
                            ...
                            label_visibility="collapsed",
                            on_change=sync_template_from_editor,
                            args=("template_checkin_reminder",),
                        )
                    with template_tabs[2]:
                        st.text_area(
                            ...
                            label_visibility="collapsed",
                            on_change=sync_template_from_editor,
                            args=("template_checkin_instructions",),
                        )
                    with template_tabs[3]:
                        st.text_area(
                            ...
                            label_visibility="collapsed",
                            on_change=sync_template_from_editor,
                            args=("template_checkout_reminder",),
                        )
            ...
            scheduling_rules = {
                "confirm_offset_days": st.session_state.get("msg_rule_confirm_offset_days", 0),
                "confirm_time": st.session_state.get("msg_rule_confirm_time", "10:00"),
                "checkin_reminder_days": st.session_state.get("msg_rule_checkin_reminder_days", 1),
                "checkin_reminder_time": st.session_state.get("msg_rule_checkin_reminder_time", "18:00"),
                "checkin_instr_time": st.session_state.get("msg_rule_checkin_instr_time", "15:00"),
                "checkout_reminder_days": st.session_state.get("msg_rule_checkout_reminder_days", 1),
                "checkout_reminder_time": st.session_state.get("msg_rule_checkout_reminder_time", "18:00"),
                "review_days_after": st.session_state.get("msg_rule_review_days_after", 1),
                "review_time": st.session_state.get("msg_rule_review_time", "12:00"),
            }
            ...
            if generate_clicked:
                ...
                template_base = {
                    "booking_confirmation": st.session_state.get("template_booking_confirmation", ""),
                    "checkin_reminder": st.session_state.get("template_checkin_reminder", ""),
                    "checkin_instructions": st.session_state.get("template_checkin_instructions", ""),
                    "checkout_reminder": st.session_state.get("template_checkout_reminder", ""),
                    "review_request": st.session_state.get("template_review_request", ""),
                }

                save_message_settings(
                    st.session_state.utente["id"],
                    {
                        "msg_rule_confirm_offset_days": st.session_state.get("msg_rule_confirm_offset_days", 0),
                        "msg_rule_confirm_time": st.session_state.get("msg_rule_confirm_time", "10:00"),
                        "msg_rule_checkin_reminder_days": st.session_state.get("msg_rule_checkin_reminder_days", 1),
                        "msg_rule_checkin_reminder_time": st.session_state.get("msg_rule_checkin_reminder_time", "18:00"),
                        "msg_rule_checkin_instr_time": st.session_state.get("msg_rule_checkin_instr_time", "15:00"),
                        "msg_rule_checkout_reminder_days": st.session_state.get("msg_rule_checkout_reminder_days", 1),
                        "msg_rule_checkout_reminder_time": st.session_state.get("msg_rule_checkout_reminder_time", "18:00"),
                        "msg_rule_review_days_after": st.session_state.get("msg_rule_review_days_after", 1),
                        "msg_rule_review_time": st.session_state.get("msg_rule_review_time", "12:00"),
                        "template_booking_confirmation": st.session_state.get("template_booking_confirmation", ""),
                        "template_checkin_reminder": st.session_state.get("template_checkin_reminder", ""),
                        "template_checkin_instructions": st.session_state.get("template_checkin_instructions", ""),
                        "template_checkout_reminder": st.session_state.get("template_checkout_reminder", ""),
            ...
            if not scheduled_df.empty and not valid.empty:
                valid_keys = set(
                    valid.apply(
                        lambda r: f"{str(r.get('guest_name', '')).strip().lower()}|{pd.to_datetime(r.get('check_in'), errors='coerce').date()}|{pd.to_datetime(r.get('check_out'), errors='coerce').date()}",
                        axis=1,
                    )
                )
            ...
            elif scheduled_df.empty:
                scheduled_df = scheduled_df.copy()
            else:
                scheduled_df = scheduled_df.iloc[0:0].copy()
                ...
                if scheduled_df.empty:
                    ...
                    upcoming_base_df = scheduled_df[
                        (
                            ...
                            & (
                                (scheduled_df["check_out_dt"] >= today_ts)
                                | (scheduled_df["send_at_dt"] >= today_ts)
                            )
                        )
                        | (
                    ...
                    if display_df.empty:
                        if mostra_storico:
                            ...
                            st.info("Non ci sono prossimi messaggi da gestire. Attiva 'Mostra storico' solo se vuoi rivedere quelli vecchi.")
                    else:
                        ...
                        with right_col:
                            ...
                            if current_msg:
                                ...
                                with info1:
                                    st.text_input("Telefono", value=str(current_msg.get("guest_phone", "") or ""), disabled=True, key=f"msg_phone_{selected_id}")
                                with info2:

...
def read_excel_safely(uploaded_file, nrows=None):
    ...
    for engine in engines_to_try:
        try:
            ...
            return pd.read_excel(uploaded_file, nrows=nrows, engine=engine)
        except Exception as e:
            last_error = e

    raise ValueError(
        "Impossibile leggere il file Excel. "
        "Per i file .xls su Streamlit Cloud assicurati che in requirements.txt ci sia anche xlrd. "
        f"Dettaglio errore: {last_error}"
    )
...
def parse_mixed_date_series(series):
    ...
    # First pass: normal strings / timestamps
    parsed = pd.to_datetime(s, errors="coerce")

    # Second pass: Excel serial numbers sometimes returned by older XLS readers
...
def detect_booking_export(columns):
    ...
    checks = [
        ...
        any("prezzo" in c or "importo" in c or "totale" in c for c in cols),
...
def load_booking_file(uploaded_file, cleaning_cost_default):
    ...
    if raw is None or raw.empty:
        raise ValueError("Il file è stato letto ma non contiene righe utili.")
    ...
    guests_col = find_col(["persone", "ospiti", "numero ospiti", "guest"])
    ...
    if missing:
        available_cols = ", ".join(raw.columns)
        raise ValueError(
            f"Nel file Booking mancano queste colonne obbligatorie: {', '.join(missing)}. "
            f"Colonne trovate: {available_cols}"
        )

    check_in_series = parse_mixed_date_series(raw[checkin_col])
    check_out_series = parse_mixed_date_series(raw[checkout_col])

    df = pd.DataFrame({
        "platform": "Booking",
        "guest_name": raw[guest_col].astype(str).str.strip(),
        "check_in": check_in_series,
        "check_out": check_out_series,
    ...
    if df.empty:
        debug_preview = pd.DataFrame({
            "guest_name_raw": raw[guest_col].astype(str).head(10),
            "check_in_raw": raw[checkin_col].head(10),
            "check_out_raw": raw[checkout_col].head(10),
            "check_in_parsed": check_in_series.head(10),
            "check_out_parsed": check_out_series.head(10),
        })
        raise ValueError(
            "Il file è stato letto ma tutte le righe sono state scartate durante la normalizzazione. "
            f"Anteprima parsing: {debug_preview.to_dict(orient='records')}"
        )

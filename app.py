...
if st.session_state.utente is None:
    ...
    if isinstance(token_qs, list):
        token_qs = token_qs[0] if token_qs else None
    token_qs = str(token_qs).strip() if token_qs else None
...
if st.session_state.get("auth_token"):
    current_session = st.query_params.get("session")
    if isinstance(current_session, list):
        current_session = current_session[0] if current_session else None
    current_session = str(current_session).strip() if current_session else None

    if current_session != st.session_state["auth_token"]:
...
with st.sidebar:
    ...
    saved_sidebar_settings = carica_sidebar_settings(st.session_state.utente["id"])
    merged_sidebar_settings = dict(saved_sidebar_settings)
    ...
    if merged_sidebar_settings != saved_sidebar_settings:

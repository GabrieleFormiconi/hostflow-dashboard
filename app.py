...
def render_profile_form(profilo, onboarding_mode=False):
    ...
    status_value = status_label_map.get(current_whatsapp_config.get("embedded_signup_status"), current_whatsapp_config.get("embedded_signup_status", "Non collegato"))
    if current_whatsapp_config.get("is_configured"):
        ...
        if current_whatsapp_config.get("uses_platform_default"):
            st.caption("Stai usando la configurazione WhatsApp di default della piattaforma. Per vendere il prodotto a clienti reali, collega qui il loro account dedicato.")
        else:
            summary_items = []
            ...
            if current_whatsapp_config.get("business_phone_number"):
                summary_items.append(f"Numero: +{current_whatsapp_config.get('business_phone_number')}")
            if current_whatsapp_config.get("phone_number_id"):
                summary_items.append(f"Phone Number ID: {current_whatsapp_config.get('phone_number_id')}")
            if current_whatsapp_config.get("token_last4"):
                summary_items.append(f"Token finale: ****{current_whatsapp_config.get('token_last4')}")
            if summary_items:
                st.caption(" · ".join(summary_items))
            if current_whatsapp_config.get("last_test_status") == "success":
                st.caption("Ultimo test invio: riuscito")
            elif current_whatsapp_config.get("last_test_status") == "failed":
                st.caption("Ultimo test invio: fallito")

    st.info("Onboarding SaaS pronto per i test: ogni cliente può salvare qui il proprio Access Token Meta e il proprio Phone Number ID. L'invio messaggi userà automaticamente le credenziali dell'account loggato.")

    with st.expander("Collega / aggiorna WhatsApp Business", expanded=not current_whatsapp_config.get("is_configured", False)):
        ...
        wa_business_phone_number = st.text_input(
            ...
            help="Numero che il cliente collega a WhatsApp Cloud API. Usa prefisso internazionale, es. +393XXXXXXXXX",
        )
        ...
        if save_whatsapp_clicked:
            ...
            if not str(wa_phone_number_id or "").strip():
                ...
                st.success("Collegamento WhatsApp salvato per questo account.")
                st.rerun()
        ...
        if disconnect_whatsapp_clicked:
            disconnect_whatsapp_config(st.session_state.utente["id"])
            st.session_state.whatsapp_config = get_whatsapp_config(st.session_state.utente["id"])
            st.info("Collegamento WhatsApp rimosso da questo account.")
            st.rerun()

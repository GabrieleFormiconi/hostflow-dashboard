...
def inizializza_sessione():
    ...
    if "file_prenotazioni_utente_id" not in st.session_state:
        st.session_state.file_prenotazioni_utente_id = None
            ...
            if submitted:
                if not email.strip():
                    ...
                    if utente:
                        ...
                        st.session_state.file_prenotazioni_nome = None
                        st.session_state.file_prenotazioni_signature = None
                        st.session_state.file_prenotazioni_utente_id = utente["id"]
...
current_user_id = st.session_state.utente["id"]
if st.session_state.get("file_prenotazioni_utente_id") != current_user_id:
    st.session_state.file_prenotazioni_virtuale = None
    st.session_state.file_prenotazioni_nome = None
    st.session_state.file_prenotazioni_signature = None
    st.session_state.file_prenotazioni_utente_id = current_user_id

if st.session_state.get("file_prenotazioni_virtuale") is None:
    file_salvato = carica_file_prenotazioni(current_user_id)
    if file_salvato is not None:
        saved_bytes = file_salvato.getvalue()
        if hasattr(file_salvato, "seek"):
            file_salvato.seek(0)
        ...
        st.session_state.file_prenotazioni_signature = hashlib.md5(
            f'{getattr(file_salvato, "name", "prenotazioni.xlsx")}|{len(saved_bytes)}'.encode("utf-8") + saved_bytes
        ).hexdigest()
    ...
    if uploaded_file_widget is not None:
        ...
        if uploaded_bytes:
            if st.session_state.get("file_prenotazioni_signature") != uploaded_signature:
                ...
                buffer_file.seek(0)
                salva_file_prenotazioni(st.session_state.utente["id"], buffer_file)
                buffer_file.seek(0)
                ...
                st.session_state.file_prenotazioni_utente_id = st.session_state.utente["id"]
                st.rerun()

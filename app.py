...
def get_runtime_secret(name, default=None):
    try:
...
cookies = EncryptedCookieManager(
    prefix="hostflow/",
    password=str(get_runtime_secret("COOKIES_PASSWORD", "hostflow-local-cookie-secret-change-me")),
)
...
.hf-onboarding-box textarea::placeholder {{
...
@media (max-width: 768px) {{
    ...
    .stTabs [aria-selected="true"] {{
        color: {COLORI["colore_primario"]} !important;
    }}
...
Sostituisci completamente il blocco del bottone login con questo.
...
if st.session_state.utente is None:
    token_cookie = cookies.get("session_token")
    if token_cookie:
        auth_data = autentica_da_token(token_cookie)
        if auth_data:

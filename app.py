...
def load_booking_file(uploaded_file, cleaning_cost_default):
    ...
    guest_col = find_col(["nome osp", "ospite", "guest name", "cliente", "guest"])
    checkin_col = find_col(["arrivo", "check-in", "check in", "data arrivo", "arrival"])
    checkout_col = find_col(["partenza", "check-out", "check out", "data partenza", "departure"])
    price_col = find_col(["prezzo", "totale", "importo totale", "totale prenotazione", "importo", "amount"])
    fee_col = find_col(["importo commissione", "commissione", "commission", "fee"])
    status_col = find_col(
        ["stato prenotazione", "booking status", "reservation status", "stato", "status"],
        exclude_fragments=["pagamento", "payment", "commission", "commissione", "camera", "stanza"],
        prefer_exact=["stato prenotazione", "booking status", "reservation status", "stato", "status"],
    )
    guests_col = find_col(["persone", "ospiti", "guest count", "numero ospiti", "adults"])
    ...
    parsed_check_in = parse_mixed_date_series(raw[checkin_col])
    parsed_check_out = parse_mixed_date_series(raw[checkout_col])
    ...
    df = df[df["guest_name"].str.lower().ne("nan")].copy()
    df = df[df["check_out"] > df["check_in"]].copy()

    if df.empty and len(raw) > 0:
        raise ValueError(
            "Il file è stato letto ma nessuna riga è risultata valida. Controlla che le date di arrivo e partenza siano valorizzate e in un formato leggibile."
        )
...
def parse_mixed_date_series(series):
    ...
    parsed_dayfirst = pd.to_datetime(text_values, errors="coerce", dayfirst=True)
    parsed_monthfirst = pd.to_datetime(text_values, errors="coerce", dayfirst=False)

    parsed = parsed_numeric.copy()
    parsed = parsed.where(parsed.notna(), parsed_dayfirst)
    parsed = parsed.where(parsed.notna(), parsed_monthfirst)

    return parsed.dt.date


def cleanup_raw_dataframe(raw):
    raw = raw.copy()
    ...
    if len(raw.columns) > 0:
        unnamed_ratio = sum(1 for c in raw.columns if str(c).strip().lower().startswith("unnamed")) / len(raw.columns)

    if unnamed_ratio >= 0.5 and len(raw) > 0:
        candidate_header = raw.iloc[0].tolist()
        raw = raw.iloc[1:].copy()
        raw.columns = [str(c).strip() for c in candidate_header]

    raw.columns = [str(c).strip() for c in raw.columns]
    raw = raw.loc[:, [str(c).strip() != "" for c in raw.columns]].copy()
    return raw

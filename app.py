...
def replace_scheduled_messages_for_user(utente_id, bookings_df, profilo, scheduling_rules=None, template_base=None):
    ...
    cur.execute(
        """
        SELECT id, booking_ref, message_type, send_at, status, message_text, sent_at, guest_phone
    ...
    existing_map = {
        (row["booking_ref"], row["message_type"]): dict(row)
    ...
    for _, row in valid.iterrows():
        for msg in build_scheduled_messages_for_booking(
            ...
            key = (msg["booking_ref"], msg["message_type"])
            ...
            if existing:
                cur.execute(
                    ...
                    SET platform = ?, guest_name = ?, guest_phone = ?, check_in = ?, check_out = ?,
                        message_text = ?, send_at = ?, channel = ?, status = 'pending',
    ...
    for row in existing_rows:
        row_key = (row["booking_ref"], row["message_type"])
        if row_key not in current_keys and row["status"] not in ["sent", "cancelled"]:
...
def ensure_template_editor_value(editor_key, persistent_key):

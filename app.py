...
def read_excel_safely(uploaded_file, nrows=None, header=0):
    ...
    for engine in ["xlrd", "openpyxl", None]:
        try:
            uploaded_file.seek(0)
            kwargs = {"nrows": nrows, "header": header}
...
def looks_like_booking_header(columns):
    cols = [normalize_header_name(c) for c in columns]
    score = 0

    if any("nome osp" in c or "guest" in c or "ospite" in c for c in cols):
        score += 1
    if any("arrivo" in c or "check in" in c for c in cols):
        score += 1
    if any("partenza" in c or "check out" in c for c in cols):
        score += 1
    if any("prezzo" in c or "importo" in c or "totale" in c for c in cols):
        score += 1

    return score >= 3


def read_booking_excel_with_header_detection(uploaded_file):
    last_df = None

    for header_row in range(0, 8):
        try:
            df_try = read_excel_safely(uploaded_file, header=header_row)
            last_df = df_try
            if looks_like_booking_header(df_try.columns):
                return df_try
        except Exception:
            continue
...
def detect_booking_export(columns):
    return looks_like_booking_header(columns)
...
def load_booking_file(uploaded_file, cleaning_cost_default):
    ...
    try:
        if ext == ".csv":
            ...
            raw = read_booking_excel_with_header_detection(uploaded_file)
        else:
    ...
    raw_columns = {normalize_header_name(c): c for c in raw.columns}

    def find_col(possible_fragments, exclude_fragments=None, prefer_exact=None):
        exclude_fragments = [normalize_header_name(x) for x in (exclude_fragments or [])]
        prefer_exact = [normalize_header_name(x) for x in (prefer_exact or [])]
        possible_fragments = [normalize_header_name(x) for x in possible_fragments]
    ...
    guest_col = find_col(["nome osp", "ospite", "guest name", "cliente"])
    ...
    price_col = find_col(["prezzo", "totale", "importo totale", "totale prenotazione", "importo"])
    fee_col = find_col(["importo commissione", "commissione", "commission", "fee"])
    status_col = find_col(
        ["stato prenotazione", "booking status", "reservation status", "stato"],
        exclude_fragments=["pagamento", "payment", "camera", "stanza"],
        prefer_exact=["stato prenotazione", "booking status", "reservation status", "stato"]
    )
    guests_col = find_col(["persone", "ospiti", "guest", "numero ospiti"])
    ...
    check_in_series = raw[checkin_col].apply(parse_booking_date)
    check_out_series = raw[checkout_col].apply(parse_booking_date)

    df = pd.DataFrame({
        "platform": "Booking",
        "guest_name": raw[guest_col].astype(str).str.strip(),
        "check_in": check_in_series.dt.date,
        "check_out": check_out_series.dt.date,
    ...
    df = df[df["check_out"] > df["check_in"]].copy()

    if df.empty and len(raw) > 0:
        raise ValueError(
            "Il file è stato letto ma nessuna riga prenotazione è risultata valida. Controlla il formato delle colonne Arrivo e Partenza nell'export Booking."
        )
...
def load_data(uploaded_file, cleaning_cost_default, import_mode):
    ...
    if ext in [".xls", ".xlsx"]:
        try:
            preview = read_booking_excel_with_header_detection(uploaded_file)
            uploaded_file.seek(0)

...
if "messaggi" in tab_map:
    with tab_map["messaggi"]:
        if df is None:
            ...
            top_action_col1, top_action_col2 = st.columns([2.2, 1])
            with top_action_col1:
                generate_clicked = st.button("Genera / aggiorna messaggi programmati", use_container_width=True, key="generate_scheduled_messages_top")
            with top_action_col2:
                show_templates_box = st.toggle("Mostra template", value=False, key="show_message_templates_toggle")

            if needs_regeneration:
                st.warning("Hai modificato regole o template. Premi 'Genera / aggiorna messaggi programmati' per aggiornare i messaggi già creati.")

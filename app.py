...
def read_excel_safely(uploaded_file, nrows=None, header=0):
    ...
    for engine in ["xlrd", "openpyxl", None]:
        try:
            ...
            last_error = e
    raise ValueError(f"Impossibile leggere il file Excel. Errore: {last_error}")
...
def detect_header_row_excel(uploaded_file, max_rows=8):
    for header_row in range(max_rows):
        try:
            preview = read_excel_safely(uploaded_file, nrows=5, header=header_row)
            cols = [normalize_column_name(c) for c in preview.columns]
            checks = [
...
def parse_booking_date_series(series):
    s = series.copy()
    out = pd.to_datetime(s, errors="coerce", dayfirst=False)

    if out.notna().sum() == 0:
        numeric = pd.to_numeric(s, errors="coerce")
        out = pd.to_datetime("1899-12-30") + pd.to_timedelta(numeric, unit="D")
...
def load_booking_file(uploaded_file, cleaning_cost_default):
    ...
    try:
        if ext == ".csv":
            ...
            header_row = detect_header_row_excel(uploaded_file)
            raw = read_excel_safely(uploaded_file, header=header_row)
        else:
            raise ValueError("Formato non supportato. Usa CSV, XLS o XLSX.")
    except Exception as e:
        raise ValueError(f"Errore durante la lettura del file: {e}")

    raw = clean_headers(raw)
    raw = raw.dropna(axis=0, how="all").dropna(axis=1, how="all")
    raw_columns = {normalize_column_name(c): c for c in raw.columns}

    def find_col(possible_fragments, exclude_fragments=None, prefer_exact=None):
        exclude_fragments = [normalize_column_name(x) for x in (exclude_fragments or [])]
        prefer_exact = [normalize_column_name(x) for x in (prefer_exact or [])]
        ...
        for frag in possible_fragments:
            frag_norm = normalize_column_name(frag)
            for low, original in raw_columns.items():
                if frag_norm in low and not any(ex in low for ex in exclude_fragments):
                    return original
        return None

    guest_col = find_col(["nome osp", "nome ospite", "ospite", "guest name", "cliente"])
    ...
    price_col = find_col(["prezzo", "totale prenotazione", "importo totale", "totale", "importo"])
    fee_col = find_col(["importo commissione", "commissione", "commission", "% commissione", "fee"])
    status_col = find_col(
    ...
    df = pd.DataFrame({
        ...
        "check_in": parse_booking_date_series(raw[checkin_col]),
        "check_out": parse_booking_date_series(raw[checkout_col]),
    ...
    df["guest_name"] = df["guest_name"].replace({"nan": "", "None": ""}).fillna("").astype(str).str.strip()
    df = df.dropna(subset=["check_in", "check_out"]).copy()
    df = df[df["guest_name"] != ""].copy()

    if df.empty:
        debug_cols = ", ".join([str(c) for c in raw.columns])
        raise ValueError(
            "Il file è stato letto ma tutte le righe sono risultate vuote dopo la normalizzazione. "
            f"Colonne rilevate: {debug_cols}"
        )
...
def load_data(uploaded_file, cleaning_cost_default, import_mode):
    ...
    if import_mode == "CSV standard":
        if ext != ".csv":
            raise ValueError("Hai selezionato CSV standard ma il file non è un CSV.")
        uploaded_file.seek(0)

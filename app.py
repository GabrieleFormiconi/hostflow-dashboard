...
def delete_scheduled_messages_for_booking(utente_id, platform, guest_name, check_in, check_out):
    booking_ref = f"{str(platform or '').strip()}|{str(guest_name or '').strip()}|{str(check_in)}|{str(check_out)}"
    ...
    cur.execute(
        ...
        WHERE utente_id = ?
          AND (
              booking_ref = ?
              OR (
                  lower(trim(coalesce(guest_name, ''))) = lower(trim(?))
...
def delete_custom_booking(utente_id, booking_id):
    ...
    cur.execute(
        """
        SELECT platform, guest_name, check_in, check_out
    ...
    if changed > 0 and booking_row:
        delete_scheduled_messages_for_booking(
        ...
        if df is None:
            ...
            for persistent_key, editor_key in template_editor_key_map.items():
                if editor_key in st.session_state:
                    st.session_state[persistent_key] = st.session_state.get(editor_key, st.session_state.get(persistent_key, saved_message_settings.get(persistent_key, "")))
                else:
            ...
            if len(valid) == 0:
                ...
                if scheduled_df.empty:
                    ...
                    if upcoming_base_df.empty:
                        upcoming_base_df = scheduled_df.copy()

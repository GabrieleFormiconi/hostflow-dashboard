...
def reimposta_password_da_email(email, nuova_password):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id FROM utenti WHERE email = ?", (email.strip().lower(),))
    row = cur.fetchone()
...
def parse_excel_date_safely(value):
    if pd.isna(value):
        return pd.NaT
...
def read_excel_safely(uploaded_file, nrows=None, header=0):
    ...
    for engine in engines_to_try:
        try:
            uploaded_file.seek(0)
            return pd.read_excel(uploaded_file, nrows=nrows, header=header, engine=engine)
        except Exception as e:
            last_error = e

    raise ValueError(
        f"Impossibile leggere il file Excel. Verifica che sia un file .xls o .xlsx valido. Errore: {last_error}"
    )
...
def detect_booking_export(columns):
    ...
    checks = [
        any("nome osp" in c or "ospite" in c for c in cols),
        any("arrivo" in c or "check-in" in c or "check in" in c for c in cols),
        any("partenza" in c or "check-out" in c or "check out" in c for c in cols),
        any("prezzo" in c or "totale" in c or "importo" in c for c in cols),
        any("persone" in c or "ospiti" in c for c in cols),
    ]
    return sum(checks) >= 4


def normalize_booking_headers(df):
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]
    return df


def read_excel_booking_with_header_detection(uploaded_file, nrows=None):
    candidates = []
    for header_row in range(0, 6):
        try:
            preview = read_excel_safely(uploaded_file, nrows=nrows, header=header_row)
            preview = normalize_booking_headers(preview)
            score = sum([
                any("nome osp" in str(c).strip().lower() or "ospite" in str(c).strip().lower() for c in preview.columns),
                any("arrivo" in str(c).strip().lower() or "check-in" in str(c).strip().lower() for c in preview.columns),
                any("partenza" in str(c).strip().lower() or "check-out" in str(c).strip().lower() for c in preview.columns),
                any("prezzo" in str(c).strip().lower() or "importo" in str(c).strip().lower() or "totale" in str(c).strip().lower() for c in preview.columns),
            ])
            candidates.append((score, header_row, preview))
        except Exception:
            continue

    if not candidates:
        raise ValueError("Impossibile leggere il foglio Excel.")

    candidates.sort(key=lambda x: (x[0], -x[1]), reverse=True)
    best_score, best_header, preview = candidates[0]
    if best_score < 3:
        raise ValueError("Excel non riconosciuto automaticamente come export Booking.")

    full_df = read_excel_safely(uploaded_file, nrows=None, header=best_header)
    return normalize_booking_headers(full_df)
...
def load_booking_file(uploaded_file, cleaning_cost_default):
    ...
    try:
        if ext == ".csv":
            ...
            raw = normalize_booking_headers(raw)
        elif ext in [".xls", ".xlsx"]:
            raw = read_excel_booking_with_header_detection(uploaded_file)
        else:
    ...
    booking_id_col = find_col(["n° di prenotazione", "numero prenotazione", "booking id", "reservation id"])
    guest_col = find_col(["nome osp", "ospite"], prefer_exact=["nome ospite(i)", "nome ospite"])
    booked_by_col = find_col(["prenotato da", "booked by"])
    checkin_col = find_col(["arrivo", "check-in", "check in", "data arrivo"])
    checkout_col = find_col(["partenza", "check-out", "check out", "data partenza"])
    price_col = find_col(["prezzo", "importo totale", "totale prenotazione", "totale", "importo"], prefer_exact=["prezzo"])
    fee_col = find_col(["importo commissione", "commissione", "commission", "fee"], prefer_exact=["importo commissione"])
    status_col = find_col(
    ...
    guests_col = find_col(["persone", "ospiti", "guest", "numero ospiti"], prefer_exact=["persone"])

    if fee_col is None:
        raw["__platform_fee__"] = 0.0
        fee_col = "__platform_fee__"

    if status_col is None:
        raw["__status__"] = "confirmed"
        status_col = "__status__"

    if guests_col is None:
        raw["__guests__"] = 1
        guests_col = "__guests__"

    if guest_col is None and booked_by_col is not None:
        guest_col = booked_by_col
    ...
    guest_series = raw[guest_col].astype(str).str.strip()
    if booked_by_col is not None:
        guest_series = guest_series.mask(guest_series.eq("") | guest_series.str.lower().eq("nan"), raw[booked_by_col].astype(str).str.strip())

    checkin_series = raw[checkin_col].apply(parse_excel_date_safely)
    checkout_series = raw[checkout_col].apply(parse_excel_date_safely)

    df = pd.DataFrame({
        "platform": "Booking",
        "booking_id": raw[booking_id_col].astype(str).str.strip() if booking_id_col is not None else "",
        "guest_name": guest_series,
        "check_in": pd.to_datetime(checkin_series, errors="coerce").dt.date,
        "check_out": pd.to_datetime(checkout_series, errors="coerce").dt.date,
    ...
    # evita empty quando l'ospite è solo nel campo "Prenotato da"
    df["guest_name"] = df["guest_name"].astype(str).str.replace(r"\s+", " ", regex=True).str.strip()

    before_rows = len(df)
    df = df.dropna(subset=["check_in", "check_out"]).copy()
    df = df[df["guest_name"].str.strip() != ""].copy()

    if df.empty and before_rows > 0:
        raise ValueError(
            "Il file è stato letto ma nessuna riga valida è rimasta dopo il parsing. "
            "Controlla formato date e colonne Arrivo/Partenza."
        )
...
def load_data(uploaded_file, cleaning_cost_default, import_mode):
    ...
    if ext in [".xls", ".xlsx"]:
        try:
            raw_preview = read_excel_booking_with_header_detection(uploaded_file, nrows=5)
            if detect_booking_export(raw_preview.columns):
                uploaded_file.seek(0)
                return load_booking_file(uploaded_file, cleaning_cost_default)
...
def load_scheduled_messages(utente_id):
    conn = get_conn()
    query = """
        SELECT id, booking_ref, platform, guest_name, check_in, check_out, message_type, send_at,
...
def render_dashboard_dataframe(df_to_show, user_id):
    ...
    with st.expander("Aggiungi prenotazione custom", expanded=False):
        ...
        if st.button("Salva prenotazione custom", use_container_width=True, key="save_custom_booking_button"):
        ...
        if custom_bookings_df.empty:
            ...
            custom_display = custom_bookings_df.copy()
            custom_display["check_in"] = pd.to_datetime(custom_display["check_in"]).dt.strftime("%d/%m/%Y")
            custom_display["check_out"] = pd.to_datetime(custom_display["check_out"]).dt.strftime("%d/%m/%Y")
        ...
        with st.form("reset_password_form"):
            ...
            if submitted:
                if not email.strip() or "@" not in email:
                    ...
                    ok, messaggio = reimposta_password_da_email(email, nuova_password)
                    if ok:
...
with st.sidebar:
    ...
    if uploaded_file_widget is not None:
        ...
        buffer_file = BytesIO(uploaded_file_widget.getvalue())
...
if "messaggi" in tab_map:
    with tab_map["messaggi"]:
        if df is None:
            ...
            st.caption("Configura qui regole e template: quando clicchi \"Genera / aggiorna messaggi programmati\", i messaggi già pianificati vengono creati o aggiornati in base alle impostazioni qui sotto.")

            st.markdown("### Regole invio automatico")
            st.caption("Per ogni messaggio imposti in modo semplice quando deve partire.")
            ...
            st.info(
                "Nome struttura, orari, Wi‑Fi, host, WhatsApp, parcheggio e tassa di soggiorno vengono presi automaticamente dalla Scheda immobile."
            )
            st.caption(
                "Puoi usare queste variabili: {nome_ospite}, {data_checkin}, {data_checkout}, {nome_struttura}, {nome_host}, "
                "{numero_whatsapp}, {checkin_da}, {checkin_fino}, {checkout_entro}, {wifi_nome}, {wifi_password}, "
                "{parcheggio_disponibile}, {tassa_soggiorno}, {testo_parcheggio}, {testo_tassa_soggiorno}"
            )

            with st.expander("Conferma prenotazione", expanded=False):
                template_booking_confirmation = st.text_area(
                    "Template conferma prenotazione",
                    key="template_booking_confirmation",
                    height=150,
                )

            with st.expander("Reminder check-in", expanded=False):
                template_checkin_reminder = st.text_area(
                    "Template reminder check-in",
                    key="template_checkin_reminder",
                    height=150,
                )

            with st.expander("Istruzioni check-in", expanded=False):
                template_checkin_instructions = st.text_area(
                    "Template istruzioni check-in",
                    key="template_checkin_instructions",
                    height=180,
                )

            with st.expander("Reminder check-out", expanded=False):
                template_checkout_reminder = st.text_area(
                    "Template reminder check-out",
                    key="template_checkout_reminder",
                    height=150,
                )

            with st.expander("Richiesta recensione", expanded=False):
                template_review_request = st.text_area(
            ...
            st.markdown("### Applica modifiche")
            if st.button("Genera / aggiorna messaggi programmati", use_container_width=True):
                ...
                if scheduled_df.empty:
                    ...
                    with filter_col1:
                        stato_filtro = st.selectbox(
                            "Filtro stato",
                            ["Tutti", "pending", "sent", "failed", "cancelled"],
                            index=0,
                    ...
                    with filter_col2:
                        tipo_filtro = st.selectbox(
                            "Filtro tipo",
                            ["Tutti"] + [label_message_type(x) for x in sorted(scheduled_df["message_type"].dropna().unique())],
                            index=0,
                    ...
                    pretty_df["check_in"] = pd.to_datetime(pretty_df["check_in"]).dt.strftime("%d/%m/%Y")
                    pretty_df["check_out"] = pd.to_datetime(pretty_df["check_out"]).dt.strftime("%d/%m/%Y")
                    ...
                    if not display_df.empty:
                        st.markdown("### Gestione singolo messaggio")

                        display_df = display_df.sort_values(["send_at", "id"]).copy()
                        message_ids = [int(x) for x in display_df["id"].tolist()]
                        labels_by_id = {
                            int(row["id"]): f'#{int(row["id"])} · {row["guest_name"]} · {label_message_type(row["message_type"])} · {pd.to_datetime(row["send_at"]).strftime("%d/%m/%Y %H:%M")}'
                        ...
                        selected_id = st.selectbox(
                            ...
                            format_func=lambda mid: labels_by_id.get(mid, str(mid)),
                        ...
                        if current_msg:
                            ...
                            info1.metric("Stato", current_msg["status"])
                            info2.metric("Canale", current_msg["channel"])
                            info3.metric("Tipo", label_message_type(current_msg["message_type"]))
                            info4.metric("Invio", pd.to_datetime(current_msg["send_at"]).strftime("%d/%m/%Y %H:%M"))

                            st.text_area(
                                "Testo del messaggio generato",

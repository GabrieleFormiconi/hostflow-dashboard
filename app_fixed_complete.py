...
def parse_flexible_date_series(series):
    ...
    if numeric_series.notna().any():
        numeric_dates = pd.to_datetime("1899-12-30") + pd.to_timedelta(numeric_series, unit="D")
        parsed = parsed.where(parsed.notna(), numeric_dates)
...
def detect_booking_export(columns):
    ...
    checks = [
        any("nome osp" in c or "ospite" in c or "guest" in c for c in cols),
...
def load_booking_file(uploaded_file, cleaning_cost_default):
    ...
    check_in_series = parse_flexible_date_series(raw[checkin_col])
    check_out_series = parse_flexible_date_series(raw[checkout_col])
    ...
    if df.empty:
        raise ValueError(
            "Il file è stato letto ma nessuna riga valida è stata riconosciuta. Controlla che le colonne Arrivo e Partenza contengano date reali."
        )
